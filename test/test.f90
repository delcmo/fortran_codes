...
!     ifl = number of fluids (restricted to 2 in the present version)
      integer(kind=4), parameter :: im=502, ieq=4, ifl=2
!
...
!     Indexes for the geometric junctions: 
      ...
      integer(kind=4), parameter:: idisc2=251 ! index at which the second segment ends
      integer(kind=4), parameter:: idisc3=276 ! index at which the third segment ends
      integer(kind=4), parameter:: idisc4=502 ! index at which the fourth segment ends
!
...
!... Grid Spacing, node positions, and cell-edge areas
      do i=1,im
        xent = xx(i)-dx/2.d0
        xsort= xx(i)+dx/2.d0
        surfent(i) = 1.d0 ! area0*(1.D0+0.5D0*DCOS(xent*2.d0*PI))
        surfsort(i)= 1.d0 ! area0*(1.D0+0.5D0*DCOS(xsort*2.d0*PI))
